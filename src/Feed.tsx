...
export const Feed: React.FC = () => {
	...
	let url = 'http://192.168.0.17:8000/';
	...
	if (!fetching) {
		...
		const listItems = posts.map((post: any) =>
			<Post _id = {post._id} link = { post.link } onClickAction={ handleDelete } title = { post.title } author = { post.author } created_at = { showDate(post.created_at) } />
		);
	    return (
		    <React.Fragment>
